...
class EPO_PMN(PPO):
        ...
        for i in track(range(self._cfgs.algo_cfgs.update_iters), description='Updating...'):
            for (
                ...
                if self._cfgs.algo_cfgs.use_cost:
                    self._update_cost_critic(obs, target_value_c, target_value_cs)
                self._update_actor(
                    obs,
                    act,
                    logp,
    ...
    def _update_actor(  # pylint: disable=too-many-arguments
        ...
        loss, info = self._loss_safety_pi_via_cost(obs, act, logp, adv_r, adv_c, adv_cs)
    ...
    def _loss_safety_pi_via_cost(
        ...
        pi_obj = surr_adv + self._cfgs.algo_cfgs.entropy_coef * distribution.entropy().mean()
        self._logger.store(**{'Train/Reward_obj': pi_obj})

        surr_cadv = (ratio * adv_c).mean()
        surr_csadv = (ratio * adv_cs).mean()
        JcExp = self._logger.get_stats('Metrics/EpCost')[0] - self._cfgs.algo_cfgs.cost_limit
